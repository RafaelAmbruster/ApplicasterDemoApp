...
public class TwitterItemAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<Tweet> original_items = new ArrayList<>();

    public TwitterItemAdapter(Activity activity, OnItemClickListener onItemClickListener) {
        ctx = activity;
    ...
    public class TwitterViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void bind(final Tweet c, final int position) {

            if (c.getUser().getProfileImageUrl() != null) {
                if (!Validate(c.getUser().getProfileImageUrl())) {
                    String URL = c.getUser().getProfileImageUrl();
                    Picasso.with(ctx).load(URL).into(iv_profile_pic);
                }
            }

            if (c.getEntities().getMedia() != null) {
                if (c.getEntities().getMedia().get(0).getMediaUrl() != null) {
                    if (!Validate(c.getEntities().getMedia().get(0).getMediaUrl())) {
                        iv_media_pic.setVisibility(View.VISIBLE);
                        String URL = c.getEntities().getMedia().get(0).getMediaUrl();
            ...
            tv_search_user.setText(c.getUser().getName());
            tv_search_followers.setText(c.getUser().getFollowersCount() + " followers");
